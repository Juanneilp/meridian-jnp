...
const SCREENER_TOOLS = new Set(["deploy_position", "check_smart_wallets_on_pool", "get_token_holders", "get_token_narrative", "get_token_info", "search_pools", "get_pool_memory", "get_wallet_balance", "get_my_positions"]);
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      const toolResults = await Promise.all(msg.tool_calls.map(async (toolCall) => {
        ...
        // If a tool automatically swapped the token (e.g. close_position dust sweeping), block manual swap_token
        if (result && result.auto_swapped) {
          firedOnce.add("swap_token");
        }
